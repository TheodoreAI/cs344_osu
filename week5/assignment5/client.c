...
ex: 
./client localhost 49123
Make sure that the server is already started.
*/
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  if (socketFD < 0){
   ...
   /*
    Call this function: STEP 2: Sets up a socket address strucutre sockaddr_in with the (IP address) and (port number). 
   */
...
*/
  ...
  /*
    Send message to server: STEP 4: send data to the server and recieve data from the server over the sockets.
    Important to check how many bytes were actually sent by send() and to handle partial sends by keep calling send() 
    until all the remaining bytes have been sent.
  */
